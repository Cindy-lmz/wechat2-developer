...
class Card extends BasicWeChat
{
    ...
    public function consume($code, $card_id = null)
    {
        ...
        $this->registerApi($url, __FUNCTION__, func_get_args());
        return $this->httpPostForJsonarr($url, $data);
    }
     ...
     * @throws Exceptions\LocalCacheException
     */
    public function updateCard($card_id,$type, array $member_card)
    {
        $url = "https://api.weixin.qq.com/card/update?access_token=ACCESS_TOKEN";
        $this->registerApi($url, __FUNCTION__, func_get_args());
        return $this->httpPostForJson($url, ['card_id' => $card_id, $type => $member_card]);
    }
     ...
     * @throws Exceptions\LocalCacheException
     */
    public function activatgeturl(array $data)
    {
