...
class BasicWeChat
{
    ...
    public function getAccessToken()
    {
        ...
        if (!empty($result['access_token'])) {
            cache($cache, $result['access_token'], 7000);
    ...
    public function setAccessToken($access_token)
    {
        ...
        $cache = $this->config->get('appid') . '_access_token';
        cache($cache, $this->access_token = $access_token);
    ...
    {
        $this->access_token = '';
        return cache($this->config->get('appid') . '_access_token',NULL);
    ...
    protected function httpPostForJson($url, array $data, $buildToJson = true)
    {
        try {
            ...
            if (!$this->isTry && in_array($e->getCode(), ['40014', '40001', '41001', '42001','40100'])) {

                [$this->delAccessToken(), $this->isTry = true];

                return call_user_func_array([$this, $this->currentMethod['method']], $this->currentMethod['arguments']);
            }
            //hulkwang 20191214 从微信返回的报错，以json形式返回
            return json_encode_ex(array('code'=>$e->getCode(),'message'=>$e->getMessage()));exit;
            //throw new InvalidResponseException($e->getMessage(), $e->getCode());
        }
    }
